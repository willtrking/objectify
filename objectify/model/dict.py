...
class ObjectifyDict(ObjectifyModel):
    ...
    def __init__(self,*args,**kwargs):
        ...
        if _exclude_from_collection is not None:
            self.__exclude_from_collection__ = frozenset(_exclude_from_collection)
        else:
            self.__exclude_from_collection__ = frozenset(self.__exclude_from_collection__)
        ...
        if isinstance(self.__exclude_empty__,list):
            self.__exclude_empty__ = frozenset(self.__exclude_empty__)
                ...
                if isinstance(val,ObjectifyObject):
                    ...
                    if isinstance(existing,ObjectifyProperty):
                        ...
                        if not isinstance(val, existing.__class__):
                            ...
                            val.__key_name__ = existing.__key_name__
    ...
    def empty(self,exclude_fetch_key=False):
        ...
        if exclude_fetch_key:
            if self.__fetch_attr__ is not None:
                exclude = frozenset([self.__fetch_attr__])
